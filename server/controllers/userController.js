          ...
          (err, result) => {
            if (err) {
              return res.status(500).send({
                message: ` Please wait for admin validation !${name}`,
              });
            }
            return res
              .status(200)
  ...
  pool.query(
    ...
    (err, result) => {
      if (err) {
        ...
        return res
          .status(500)
          .send({ message: `ERROR IN FETCHING COACH SESSION LIST ` });
      }
  ...
  pool.query(
    ...
    (err, result) => {
      if (err) {
        // Error when sending db request
        return res.status(500).send({
          message: `ERROR IN DELETING SESSION WITH ID ${deletedSessionId} RETURNING *`,
        });
      }
      ...
      if (result.rows.length == 0) {
        return res
          .status(500)
          .send({ message: ` SESSION WITH ID ${deletedSessionId} NOT FOUND ` });
      }
      const deletedSession = result.rows[0];
      return res.status(200).send({
        message: ` SESSION ${deletedSession.session_id} DELETED SUCCEFFULLY `,
      });
    }
  );
};
    ...
    (err, result) => {
      if (err) {
        ...
        if (err.code === "23505") {
          return res
            .status(409)
            .send({ message: ` SESSION WITH THE SAME DATE ALLREADY EXISTS ` });
        } else {
          return res
            .status(500)
            .send({ message: `ERROR IN ADDING NEW SESSION` });
        }
      }
      const addedSession = result.rows[0];
      console.log(addedSession);
      return res.status(200).send({
        message: ` SESSION ADDED SUCCESSFFULY WITH ID `,
        session_id: addedSession.session_id,
      });
    }
  );
};
...
editCoachSession = (req, res) => {
  ...
  pool.query(
    ...
    (err, result) => {
      ...
      return res
        .status(200)
        .send({ message: ` SESSION UPDATED SUCCESSFFULY ` });
    }
  );
};
    ...
    (err, result) => {
      if (err) {
        ...
        if (err.code === "23505") {
          return res
            .status(409)
            .send({ message: `USER ALREADY REGISTERED IN THIS SESSION` });
        } else {
          return res
            .status(500)
            .send({ message: `ERROR ON REGISTERING USER IN THE SESSION` });
        }
      }
      return res
        .status(200)
        .send({ message: ` USER REGISTERED IN SESSION SEUCCESSFFULY ` });
    }
  );
};
  ...
  pool.query(
    ...
    (err, result) => {
      if (err) {
        ...
        return res
          .status(500)
          .send({ message: `ERROR ON GETTING USER VALID SESSIONS` });
      }
  ...
  pool.query(
    ...
    (err, result) => {
      if (err) {
        ...
        return res
          .status(500)
          .send({ message: `ERROR ON GETTING USERS BY LESSON` });
      }
  ...
  pool.query(
    ...
    (err, result) => {
      if (err) {
        ...
        return res
          .status(500)
          .send({ message: `ERROR ON VALIDATING USER'S SESSION BY THE COACH` });
      }
  ...
  pool.query(
    ...
    (err, result) => {
      if (err) {
        ...
        console.log(err);
        return res.status(500).send({
          message: `ERROR ON INVALIDATING USER'S SESSION BY THE COACH`,
        });
      }
...
getSessionsTypes = (req, res) => {
  pool.query(`SELECT * FROM session_type`, (err, result) => {
    if (err) {
      // Error when sending db request
      console.log(err.code);
      console.log(err);
      return res
        .status(500)
        .send({ message: `ERROR ON GETTING SESSION'S TYPES` });
    } else {
  ...
  pool.query(
    ...
    (err, result) => {
      if (err) {
        ...
        return res
          .status(500)
          .send({ message: `ERROR ON GETTING SESSION'S TITLES` });
      } else {
